...
For every additional `Handicap Index` point larger than the average `Handicap Index`, `Gross Score` increases by  **`r nlme::fixed.effects(gross_lmer) |> as.data.frame() |> dplyr::select(dplyr::contains('Handicap')) |> unlist() |> as.numeric() %>% round(., 2)`** strokes.
