...
```{r Export Model Results to db, include = T, message = F, warning = F, eval = F, echo = T}
...
For every additional `Handicap Index` point worse (higher) than my average `Handicap Index`, my expected `Gross Score` increases by **`r nlme::fixed.effects(gross_lmer) |> as.data.frame() |> tibble::rownames_to_column('coef') |> dplyr::filter(grepl(coef, pattern = 'Index.$')) |> dplyr::select(2) |> unlist() |> as.numeric() %>% round(., 2) |> abs()`** strokes.
...
+ This makes sense because `Gross Score` is used to directly determine `Handicap Index` and is positively correlated:
  ...
  + The effect is significant (**`t value` = `r stats::coef(summary(gross_lmer)) |> as.data.frame() |> tibble::rownames_to_column('coef') |> dplyr::filter(grepl(coef, pattern = 'Index.$')) |> dplyr::select(dplyr::contains("value")) |> unlist() |> as.numeric() %>% round(., 2)`**; significance : abs(**t value**) > 1)
...
For every additional `Handicap Index` point worse (higher) than my average `Handicap Index` **along with** every additional `course_rating` point harder (above) the average course rating, the expected increase in my `Gross Score` is reduced by **`r nlme::fixed.effects(gross_lmer) |> as.data.frame() |> tibble::rownames_to_column('coef') |> dplyr::slice_tail() |>  dplyr::select(2) |> unlist() |> as.numeric() %>% abs() %>% round(.,2)`** strokes relative to what the two effects would contribute independently.
