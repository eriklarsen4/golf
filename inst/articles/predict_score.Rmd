...
if (identical(fmt, "html")) {
  cat('### Code  Environment Details {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Code  Environment Details</strong></summary>\n')
} else {
  cat('### Code  Environment Details\n')
}
```
...
if (identical(fmt, "html")) {
  cat('### Code for Summarizing Metrics {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Code for Summarizing Metrics</strong></summary>\n')
} else {
  cat('### Code for Summarizing Metrics\n')
}
```
...
if (identical(fmt, "html")) {
  cat('### Code for Computing Advanced Metrics {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Code for Computing Advanced Metrics</strong></summary>\n')
} else {
  cat('### Code for Computing Advanced Metrics\n')
}
```
...
if (identical(fmt, "html")) {
  cat('### Advanced Metrics Snapshot {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Advanced Metrics Snapshot</strong></summary>\n')
} else {
  cat('### Advanced Metrics Snapshot\n')
}
```
...
if (identical(fmt, "html")) {
  cat('### Group Metrics {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Group Metrics</strong></summary>\n')
} else {
  cat('### Group Metrics\n')
}
```
...
if (identical(fmt, "html")) {
  cat('### Model Interpretations {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Model Interpretations</strong></summary>\n')
} else {
  cat('### Model Interpretations\n')
}
```

#### Big Picture: Handicap Index as a Fixed Effect Predicting Gross Score

The model's aggregate average `Gross Score` (**`(Intercept)` `Estimate` of `Fixed effects`**) is **`r nlme::fixed.effects(gross_lmer)['(Intercept)'] |> as.numeric() %>% round(.,2)`**. This is model's estimated average first `Gross Score`.

My average `Gross Score`, however, is **`r scores_sum |> dplyr::ungroup() |> dplyr::rename(Gross_Score = 25) |> dplyr::summarize(avg_gross = mean(Gross_Score)) |> unlist() |> as.numeric()`**. 

For every additional `Handicap Index` point larger than the average `Handicap Index`, my `Gross Score` increases by **`r nlme::fixed.effects(gross_lmer) |> as.data.frame() |> tibble::rownames_to_column('coef') |> dplyr::filter(grepl(coef, pattern = 'Hand')) |> dplyr::select(2) |> unlist() |> as.numeric() %>% round(., 2)`** strokes.
...
+ While this makes sense, I wonder whether `Handicap Index` should have a larger `Fixed effect` `Estimate`, which essentially asks, "how much does this variable predict the target (`Gross Score`, here)?"
  ...
  + The effect is significant (**`t value` = ** **`r stats::coef(summary(gross_lmer)) |> as.data.frame() |> tibble::rownames_to_column('coef') |> dplyr::filter(grepl(coef, pattern = 'Ha')) |> dplyr::select(dplyr::contains("value")) |> unlist() |> as.numeric() %>% round(., 2)`**; significance : abs(**t value**) > 1)
...
For every additional `course_rating` point (aka, a stroke) greater than the average `course_rating` (~69-70 strokes in this dataset), `Gross Score` increases by **`r nlme::fixed.effects(gross_lmer)[3] |> as.numeric() %>% round(.,2)`** strokes.
...
The `course` also has an effect on `Gross Score`: I play more consistently at some courses than others, as suggested by the large variance of **`r vc_df |> dplyr::filter(grp == 'course' & var1 == '(Intercept)' & is.na(var2)) |> dplyr::select(vcov) |> unlist() |> as.numeric() %>% round(., 2)`** strokes
...
if (identical(fmt, "html")) {
  cat('### Predict the Next Round {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Predict the Next Round</strong></summary>\n')
} else {
  cat('### Predict the Next Round\n')
}
```
...
if (identical(fmt, "html")) {
  cat('### Plot the Model {.tabset .tabset-pills .tabset-fade}\n')
} else if (identical(fmt, "markdown")) {
  cat('<details><summary><strong>Plot the Model</strong></summary>\n')
} else {
  cat('### Plot the Model\n')
}
```
