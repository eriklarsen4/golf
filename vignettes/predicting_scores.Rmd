...
round_date <- lubridate::as_date('2026-01-25')
index <- 10.2
course <- "Randolph"
tees <- 'combo'
...
The aggregate average `Gross Score` (intercept of `Fixed effects`) is **`r nlme::fixed.effects(gross_lmer)[1] |> as.numeric() %>% round(.,2)`** (yikes, that's bad). 

+ For every **`r nlme::fixed.effects(gross_lmer)[2] |> as.numeric() %>% round(., 2)`** additional strokes relative to the average `course_rating` (how difficult the course is), the `Gross Score` **increases by 1 stroke**.
...
The **`r nlme::fixed.effects(gross_lmer)[3] |> as.numeric() %>% round(.,2)`** `Fixed effect estimate` of `days` on `Gross Score` means that, for every additional day, `Gross Score` decreases by **`r nlme::fixed.effects(gross_lmer)[3] |> as.numeric() %>% round(.,2) |> abs()`** strokes; the `Gross Score` is getting smaller over time.
...
scores_sum |> 
  ...
  ggplot2::labs(title = paste0('Model-Predicted Gross Scores\n(colored lines, point with text)\noverlaid on Actual Gross Scores\nas of ', Sys.Date()),
                color = 'Course') +
  ggplot2::theme_bw() +
