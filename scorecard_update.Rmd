...
round_tees <- 'combo'
hole_scores <- c(5, 4, 5, 5, 4, 4, 5, 3, 5, 5, 4, 6, 5, 4, 3, 5, 6, 4)
FIRs <- c(0, 1, rep(0, 7),
          rep(0, 3), 1, rep(0, 4), 1)
GIRs <- c(0, 1, rep(0, 3), 1, rep(0, 3),
          rep(0, 3), 1, 0, 1, 1, 0, 1)
putts_rec <- c(2, 2, 1, 2, 1, 3, 1, 1, 1,
               2, 1, 3, 3, 1, 2, 2, 1, 2)
chips_rec <- c(0, 0, 1, 1, 1, 0, 2, 1, 2,
               0, 2, 1, 0, 1, 0, 1, 2, 0)
penalties_rec <- c(rep(0, 18))
tee_clubs <- c('4', '4', 'D', 'D', 'D', '9', 'D', '7', 'D',
               'D', '7', 'D', 'D', 'D', '7', 'D', 'D', '4')
index <- 10.4
```
...
scores_sum <- scores |>
  dplyr::mutate(date_course = paste0(date,
                ...
                `Iron FIRs` = dplyr::case_when(par > 3 &
                                                 grepl(tee_club, pattern = '4|5') |> any() &
                                                 FIR == 1 ~ 1,
                                               TRUE ~ 0),
                `Iron FIR opps` = dplyr::case_when(par > 3 &
                                                     grepl(tee_club, pattern = '4|5') |> any() ~ 1,
                                                   TRUE ~ 0),
                ...
                updown_conv = dplyr::case_when(GIR == 0 &
                                                 par == gross ~ 1,
                                               TRUE ~ 0),
                updown_att = dplyr::case_when(GIR == 0 &
                                                `chips+putts` >= (par - 2) ~ 1,
                                              TRUE ~ 0)
                ) |> 
  ...
  dplyr::summarize(
                   ...
                   `UpDown%` = round(((sum(updown_conv, na.rm = T)/sum(updown_att, na.rm = T))*100), 1),
...
scoring_metrics |> 
  ...
  ggplot2::theme(panel.grid.minor = ggplot2::element_blank(),
                 ...
                 axis.text.x = ggplot2::element_text(face = 'bold', angle = 270, hjust = 0, vjust = 0),
...
```{r Model predictions, include = T, message = F, warning = F, echo = T}
lme4::glmer(data = scores_sum |> 
              dplyr::ungroup() |> 
              dplyr::mutate(course_rating = dplyr::case_when(grepl(date_course, pattern = 'Sewailo') ~ 68.9,
                                                             TRUE ~ course_rating),
                            course = gsub(date_course, pattern = '[0-9]|\\-|\\\n|\\.', replacement = '')),
            formula = `Gross Score` ~ `Handicap Index` + course + course_rating + (1|`doubles+`) + (1|bogies) + (1|birdies) + (1|pars) + (1|`GIR%`) + (1|`Par 3 GIRs`) + (1|`Iron FIR%`) + (1|`Driver FIR%`))


# Predicting Gross Score
lme4::lmer(data = scores_sum |> 
              dplyr::ungroup() |> 
              dplyr::mutate(course_rating = dplyr::case_when(grepl(date_course, pattern = 'Sewailo') ~ 68.9,
                                                             TRUE ~ course_rating),
                            course = gsub(date_course, pattern = '[0-9]|\\-|\\\n|\\.', replacement = '')),
            formula = `Gross Score` ~ `Handicap Index` + course + course_rating + (1|`doubles+`) 
           # + (1|bogies) + (1|birdies) + (1|pars) + (1|`GIR%`) 
           + (1|`Par 3 GIRs`) 
           # + (1|`Iron FIR%`) + (1|`Driver FIR%`)
           )
...
lme4::lmer(data = scores_sum |> 
              ...
              dplyr::mutate(course_rating = dplyr::case_when(grepl(date_course, pattern = 'Sewailo') ~ 68.9,
                                                             TRUE ~ course_rating),
                            course = gsub(date_course, pattern = '[0-9]|\\-|\\\n|\\.', replacement = '')),
           formula = `Handicap Index` ~ `Gross Score` 
